...
// Parent (Ana menü) listesi
$parentMenus = $db->query("SELECT id, name FROM menus WHERE parent_id = 0 ORDER BY sort_order ASC, name ASC")->fetchAll();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name)) {
        ...
        $stmt = $db->prepare("INSERT INTO menus (name, link, parent_id, sort_order, status) VALUES (?, ?, ?, ?, ?)");
        if ($stmt->execute([$name, $link, $parent_id, $sort_order, $status])) {
...
<div class="p-6 max-w-2xl">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-gray-800">Yeni Menü Öğesi Ekle</h2>
    ...
    <form method="POST" class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <div class="mb-4">
            ...
            <input type="text" name="name" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" required placeholder="Örn: Ürünler">
        </div>
        <div class="mb-4">
            <label class="block text-sm font-medium text-gray-700 mb-2">Menü Linki</label>
            <input type="text" name="link" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Örn: /collections/su-aritma-cihazlari.php">
        </div>
        <div class="mb-4">
            ...
            <select name="parent_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none">
                <option value="0">--- YENİ BİR ANA BAŞLIK OLUŞTUR ---</option>
                <?php foreach ($parentMenus as $pMenu): ?>
                    <option value="<?php echo $pMenu['id']; ?>"><?php echo htmlspecialchars($pMenu['name']); ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <div class="mb-4">
        ...
        <div class="flex items-center justify-between mb-6">
            ...
            <button type="submit" class="bg-blue-600 hover:text-blue-700 text-white px-8 py-2 rounded-lg transition shadow-lg">Menü Öğesini Ekle</button>
        </div>
    </form>
</div>

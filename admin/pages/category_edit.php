...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($name)) {
        ...
        $update = $db->prepare("UPDATE categories SET name = ?, slug = ?, status = ? WHERE id = ?");
        if ($update->execute([$name, $slug, $status, $id])) {
...
<div class="p-6 max-w-2xl">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-gray-800">Kategoriyi Düzenle</h2>
    ...
    <form method="POST" class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <div class="mb-4">
            ...
            <input type="text" name="name" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" required value="<?php echo htmlspecialchars($category['name']); ?>">
        </div>
        <div class="mb-4">
            <label class="block text-sm font-medium text-gray-700 mb-2">Slug</label>
            <input type="text" name="slug" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" value="<?php echo htmlspecialchars($category['slug']); ?>">
        </div>
        <div class="flex items-center justify-between mb-6">
            <label class="flex items-center cursor-pointer">
                <input type="checkbox" name="status" class="w-5 h-5 text-blue-600 rounded" <?php echo $category['status'] ? 'checked' : ''; ?>>
                <span class="ml-2 text-gray-700">Aktif Kategori</span>
            </label>
            <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white px-8 py-2 rounded-lg transition shadow-lg">Güncelle</button>
        </div>
    </form>
</div>

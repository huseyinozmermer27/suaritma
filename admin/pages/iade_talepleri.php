...
$sql = "SELECT * FROM iade_talepleri";
if ($durum !== 'Hepsi') {
    $sql .= " WHERE iade_durumu = " . $db->quote($durum);
}
...
<div class="container mx-auto px-4 py-6">
    <div class="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
        <h1 class="text-2xl font-bold text-gray-800">İade Talepleri</h1>
        <div class="flex flex-wrap gap-2">
            <?php foreach ($durumlar as $d): ?>
                <a href="?page=iade_talepleri&filtre=<?php echo $d; ?>" 
    ...
    <div class="bg-white rounded-xl shadow border border-gray-100 overflow-hidden">
        <table class="w-full text-left">
            <thead class="bg-gray-50 border-b">
                <tr>
                    <th class="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">Talep ID</th>
                    <th class="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">Müşteri</th>
                    <th class="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">Sipariş No</th>
            ...
            <tbody class="divide-y divide-gray-100">
                <?php foreach ($talepler as $t): 
                    ...
                    $badgeClass = $colors[$t['iade_durumu']] ?? 'bg-gray-100 text-gray-800';
                ?>
                <tr class="hover:bg-gray-50 transition">
                    ...
                    <td class="px-6 py-4 text-gray-600"><?php echo htmlspecialchars($t['siparis_no']); ?></td>
                    <td class="px-6 py-4">
                        <span class="px-3 py-1 text-xs font-semibold rounded-full <?php echo $badgeClass; ?>">
                            <?php echo $t['iade_durumu']; ?>
                        </span>
                    </td>
                    <td class="px-6 py-4 text-right">
                        <a href="?page=iade_edit&id=<?php echo $t['id']; ?>" class="text-blue-600 hover:text-blue-900 font-medium text-sm">Düzenle</a>
                    </td>
                </tr>
                <?php endforeach; ?>
            </tbody>

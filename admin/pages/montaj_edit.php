<?php
$id = $_GET['id'] ?? 0;
$t = $db->query("SELECT * FROM montaj_talepleri WHERE id = $id")->fetch();
...
<div class="container mx-auto px-4 py-8 max-w-2xl">
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="bg-gray-50 px-6 py-4 border-b border-gray-200 flex justify-between items-center">
            <h2 class="text-lg font-bold text-gray-800">Montaj Talebi Düzenle <span class="text-gray-500 font-mono">#<?php echo $t['id']; ?></span></h2>
            <a href="?page=montaj_talepleri" class="text-sm text-gray-500 hover:text-gray-800">Geri Dön</a>
        </div>
        
        <form action="montaj_islem.php" method="POST" class="p-6 space-y-6">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" class="mt-1 block w-full rounded-md border-gray-300 bg-gray-50 px-3 py-2 text-gray-600" value="<?php echo htmlspecialchars($t['telefon']); ?>" disabled>
                </div>
                <div>
                    <label class="block text-xs font-bold text-gray-500 uppercase">Adres</label>
                    <input type="text" class="mt-1 block w-full rounded-md border-gray-300 bg-gray-50 px-3 py-2 text-gray-600" value="<?php echo htmlspecialchars($t['adres']); ?>" disabled>
                </div>
            </div>
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700">Montaj Durumu</label>
                <select name="montaj_durumu" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 p-2.5">
                    <?php foreach(['Beklemede', 'Onaylandı', 'Reddedildi', 'Tamamlandı'] as $s): ?>
                        <option value="<?php echo $s; ?>" <?php echo ($t['montaj_durumu'] == $s) ? 'selected' : ''; ?>><?php echo $s; ?></option>
                    <?php endforeach; ?>
                </select>
            </div>

...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] == 0) {
        if (file_exists('../assets/img/slider/' . $s['image'])) unlink('../assets/img/slider/' . $s['image']);
    ...
    $db->prepare("UPDATE slider SET title=?, subtitle=?, image=?, link=?, sort_order=? WHERE id=?")
       ->execute([$title, $subtitle, $image, $link, $sort, $id]);
    redirect('index.php?page=sliders');
}
...
<div class="p-6 max-w-2xl">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-gray-800">Slaytı Düzenle</h2>
    ...
    <form method="POST" enctype="multipart/form-data" class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <div class="mb-4">
            ...
            <input type="text" name="title" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" value="<?php echo htmlspecialchars($s['title']); ?>">
        </div>
        <div class="mb-4">
            <label class="block text-sm font-medium text-gray-700 mb-2">Alt Başlık</label>
            <input type="text" name="subtitle" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" value="<?php echo htmlspecialchars($s['subtitle']); ?>">
        </div>
        <div class="mb-4">
            <label class="block text-sm font-medium text-gray-700 mb-2">Mevcut Görsel</label>
            <img src="../assets/img/slider/<?php echo htmlspecialchars($s['image']); ?>" class="w-32 h-20 object-cover rounded-lg border mb-4">
        ...
        <div class="mb-4">
            ...
            <select name="link" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none">
                ...
                <?php foreach ($available_links as $link_path => $link_name): ?>
                    <option value="<?php echo htmlspecialchars($link_path); ?>" <?php echo ($link_path === $s['link']) ? 'selected' : ''; ?>><?php echo htmlspecialchars($link_name); ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <div class="mb-6">
            <label class="block text-sm font-medium text-gray-700 mb-2">Sıralama</label>
            <input type="number" name="sort_order" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" value="<?php echo $s['sort_order']; ?>">
        </div>

...
<section class="relative bg-primary text-white py-48 bg-cover bg-center" style="background-image: url('<?php echo $base_url; ?>../views/frontend/products/en-iyi-su-aritma-filtreleri-sumosuda.jpg');">
    ...
    <div class="container mx-auto px-4 relative z-10 h-full flex items-center">
        <div class="md:w-1/2">
            <h1 class="text-3xl md:text-4xl font-bold mb-4 whitespace-nowrap">Su Arıtma Filtreleri</h1>
            <p class="text-lg text-gray-200">Suyunuzun kalitesini korumak için en iyi filtre seçenekleri.</p>
        </div>
    </div>
    ...
    <div class="absolute bottom-6 right-6 z-20 bg-white text-gray-900 p-6 rounded-2xl shadow-xl max-w-xs">
        ...
        <?php
        $bestSeller = $db->query("SELECT * FROM products WHERE category_id = 6 ORDER BY sales_count DESC LIMIT 1")->fetch();
        if ($bestSeller): ?>
...
<main class="container mx-auto px-4 py-12">
    ...
    <div class="mt-20 bg-gray-50 py-16 px-8 rounded-3xl">
        <div class="max-w-4xl mx-auto space-y-6 text-gray-700">
            <h2 class="text-3xl font-bold text-gray-900 mb-6">Su Arıtma Filtreleri Hakkında</h2>
            <p>Filtreler, su arıtma cihazlarının kalbidir. Zamanla kirlenen filtrelerin düzenli olarak değiştirilmesi, suyun kalitesini ve cihazın ömrünü doğrudan etkiler.</p>
        </div>
    </div>
</main>
